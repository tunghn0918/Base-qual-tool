...
// ── API PROXY (calls our Vercel serverless function, never Airtable directly) ──
async function atFetch(method, queryParams = {}, body = null) {
  const qs = new URLSearchParams(queryParams).toString();
  const url = "/api/airtable" + (qs ? `?${qs}` : "");
...
async function createDeal(deal) {
  const data = await atFetch("POST", {}, { records: [{ fields: toFields(deal) }] });
  return { ...deal, id: data.records[0].id };
}

async function updateDeal(deal) {
  await atFetch("PATCH", {}, { records: [{ id: deal.id, fields: toFields(deal) }] });
}
...
const CRITERIA = [
  { group:"Pain & Business Impact", source:"SPIN · SPICED · MEDDIC · BANT", items:[
    ...
    { id:"situation", label:"Situation / context", source:"SPIN — Situation",
      stages:{ lead:"gate", qualified:"verify" },
      detail:[
        { id:"tools",      label:"Current tools",     type:"textarea", placeholder:"e.g. Excel, MISA, Zalo..." },
        { id:"structure",  label:"Team structure",    type:"textarea", placeholder:"Key depts, sizes..." },
    ...
    { id:"corepain", label:"Core pain identified", source:"SPIN · SPICED · MEDDIC",
      ...
      detail:[
        { id:"pain",       label:"Pain in client's words", type:"textarea", placeholder:"Quote or paraphrase..." },
    ...
    { id:"implication", label:"Pain implication", source:"SPIN — Implication",
      ...
      detail:[
        ...
        { id:"confidence", label:"Confidence",         type:"select",   options:["High","Medium","Low"] },
        { id:"date",       label:"Date captured",      type:"date" },
        { id:"notes",      label:"Notes",              type:"textarea", placeholder:"Context..." },
      ]},
    { id:"impact", label:"Quantified business impact", source:"MEDDIC — Metrics · SPICED",
      stages:{ meeting:"build", poc:"verify", nego:"track" },
      detail:[
        { id:"m1",        label:"Key metric #1",      type:"text",     placeholder:"e.g. 2 hrs/day saved" },
        { id:"m2",        label:"Key metric #2",      type:"text",     placeholder:"e.g. 15% payroll error → 0" },
        { id:"m3",        label:"Key metric #3",      type:"text",     placeholder:"e.g. Hiring 45 → 20 days" },
        { id:"validated", label:"Validated?",         type:"select",   options:["Client confirmed","AE estimated","PoC validated","Not quantified"] },
        { id:"date",      label:"Date captured",      type:"date" },
    ...
    { id:"criticalevent", label:"Critical event / trigger", source:"SPICED — Critical event",
      ...
      detail:[
        { id:"event",     label:"Forcing event",      type:"text",     placeholder:"e.g. New CEO, audit Q3..." },
        { id:"eventdate", label:"Event date",         type:"date" },
        { id:"type",      label:"Event type",         type:"select",   options:["Regulatory","Leadership change","Rapid growth","New office","System failure","Competitive pressure","End of contract","Other"] },
    ...
    { id:"timeline", label:"Timeline / urgency", source:"BANT · SPICED",
      ...
      detail:[
        { id:"golive",    label:"Desired go-live",        type:"date" },
        { id:"urgency",   label:"Urgency level",          type:"select",   options:["This month","1–3 months","3–6 months","6–12 months","Not decided"] },
        { id:"driver",    label:"What drives timeline?",  type:"textarea", placeholder:"Budget cycle, event..." },
        { id:"realistic", label:"Is it realistic?",       type:"select",   options:["Yes — confirmed","Likely","Unclear","No — needs reset"] },
        { id:"date",      label:"Date captured",          type:"date" },
    ...
    { id:"pocjustify", label:"PoC necessity justified", source:"Reduce PoC frequency",
      ...
      detail:[
        ...
        { id:"reason",    label:"Reason",           type:"textarea", placeholder:"What doubt makes it necessary?" },
        { id:"scope",     label:"Agreed scope",     type:"textarea", placeholder:"What tested, duration, success criteria..." },
  ...
  { group:"Stakeholders & Authority", source:"MEDDIC · BANT", items:[
    { id:"ecobuyer", label:"Economic buyer ID", source:"MEDDIC — Econ. buyer",
      ...
      detail:[
        ...
        { id:"engaged",   label:"Engaged?",         type:"select", options:["Yes — in person","Yes — call/email","Indirect","Not yet"] },
        { id:"stance",    label:"Stance",           type:"select", options:["Strongly supportive","Neutral","Skeptical","Unknown"] },
        { id:"confidence",label:"Confidence",       type:"select", options:["High","Medium","Low"] },
        { id:"date",      label:"Date identified",  type:"date" },
    ...
    { id:"committee", label:"Decision committee map", source:"MEDDIC — Decision process",
      stages:{ qualified:"build", meeting:"verify", poc:"track", nego:"watch" },
      detail:[
        { id:"members",   label:"Members (name · role · influence)", type:"textarea", placeholder:"1. Nguyen Van A — CEO — Final\n2. ..." },
        { id:"process",   label:"How they decide",                   type:"textarea", placeholder:"IT evaluates → dept head → CEO" },
        { id:"gaps",      label:"Unknown stakeholders?",             type:"textarea", placeholder:"Who haven't we reached?" },
        { id:"date",      label:"Date mapped",                       type:"date" },
        { id:"notes",     label:"Notes",                             type:"textarea", placeholder:"Dynamics, alliances..." },
      ]},
    { id:"deccriteria", label:"Decision criteria", source:"MEDDIC — Dec. criteria",
      stages:{ meeting:"gate", poc:"verify", nego:"track" },
      detail:[
        { id:"criteria",  label:"Decision criteria",   type:"textarea", placeholder:"Ease of use, support, price..." },
        { id:"musthave",  label:"Must-haves",          type:"textarea", placeholder:"Non-negotiable requirements..." },
        { id:"nicehave",  label:"Nice-to-haves",       type:"textarea", placeholder:"What would accelerate decision?" },
        { id:"compete",   label:"Competitors?",        type:"select",   options:["Yes — known","Yes — unknown","No — Base only","Unknown"] },
        { id:"date",      label:"Date captured",       type:"date" },
    ...
    { id:"champion", label:"Champion identified", source:"MEDDIC — Champion",
      ...
      detail:[
        ...
        { id:"motivation",label:"Why championing?",   type:"textarea", placeholder:"Personal stake?" },
        { id:"access",    label:"Access level",       type:"select", options:["Speaks to decision-maker","On committee","Influential peer","Working-level only"] },
        { id:"date",      label:"Date identified",    type:"date" },
        { id:"notes",     label:"Notes",              type:"textarea", placeholder:"Risks?" },
      ]},
    { id:"champstrength", label:"Champion strength", source:"Influence & engagement",
      ...
      detail:[
        { id:"influence", label:"Influence",      type:"select",   options:["High — drives decisions","Medium — respected","Low — limited","Unknown"] },
        { id:"advocacy",  label:"Advocacy",       type:"select",   options:["Proactively selling","Responsive","Passive","Declining"] },
        { id:"evidence",  label:"Evidence",       type:"textarea", placeholder:"e.g. Set up CEO meeting..." },
        { id:"risk",      label:"Risk",           type:"select",   options:["Low","Medium","High","Critical"] },
        { id:"date",      label:"Date assessed",  type:"date" },
        { id:"notes",     label:"Notes",          type:"textarea", placeholder:"Assessment..." },
      ]},
  ]},
  { group:"Budget & Payment Health", source:"BANT · AE revenue = actual payment", items:[
    { id:"budget", label:"Budget existence", source:"BANT — Budget",
      ...
      detail:[
        { id:"status",  label:"Budget status",     type:"select",   options:["Allocated","Needs requesting","Annual planning","Unknown","Declined to share"] },
        ...
        { id:"source",  label:"Source",            type:"text",     placeholder:"Client stated, inferred..." },
        { id:"date",    label:"Date captured",     type:"date" },
        { id:"notes",   label:"Notes",             type:"textarea", placeholder:"Willingness signals..." },
      ]},
    { id:"budgetctrl", label:"Budget controller", source:"Who holds the purse",
      ...
      detail:[
        { id:"name",     label:"Name",              type:"text",   placeholder:"Full name" },
        { id:"title",    label:"Title / role",      type:"text",   placeholder:"e.g. CFO..." },
        ...
        { id:"date",     label:"Date identified",   type:"date" },
        { id:"notes",    label:"Notes",             type:"textarea", placeholder:"Supportive of spend?" },
      ]},
    { id:"payprocess", label:"Internal payment process", source:"Who approves release",
      stages:{ poc:"build", nego:"verify", payment:"gate" },
      detail:[
        { id:"approver", label:"Who approves?",     type:"text",     placeholder:"Name and role" },
        { id:"steps",    label:"Steps",             type:"textarea", placeholder:"Finance → Director → transfer..." },
        { id:"duration", label:"Processing time",   type:"text",     placeholder:"e.g. 5–10 business days" },
        { id:"docs",     label:"Docs required",     type:"textarea", placeholder:"Contract, invoice, PO..." },
        { id:"date",     label:"Date mapped",       type:"date" },
        { id:"notes",    label:"Notes",             type:"textarea", placeholder:"Risks?" },
      ]},
    { id:"paycommit", label:"Payment commitment & timeline", source:"Verbal → written → executed",
      ...
      detail:[
        ...
        { id:"written",   label:"Written confirmation", type:"select",   options:["Contract signed","Email confirmed","Pending","Not yet"] },
        ...
        { id:"notes",     label:"Notes",                type:"textarea", placeholder:"Delays, next actions..." },
      ]},
  ]},
];
...
function countFilled(deal) {
  ...
  CRITERIA.forEach(g=>g.items.forEach(c=>{
    STAGE_IDS.forEach(sid=>{ if(c.stages[sid]){total++; if((deal.criteria[c.id]?.stages?.[sid]?.note||"").trim())filled++;} });
  }));
  return {filled,total};
}
      ...
      <div style={{background:"#fff",borderRadius:16,width:"100%",maxWidth:560,boxShadow:"0 24px 80px rgba(0,0,0,0.18)",overflow:"hidden",marginTop:8}}>
        <div style={{background:"#0F172A",padding:"20px 24px"}}>
          ...
          <div style={{display:"flex",gap:6,marginTop:14,flexWrap:"wrap"}}>
            {STAGE_IDS.map(sid=>{const type=criterion.stages[sid];if(!type)return null;const s=STAGES.find(x=>x.id===sid);return(
              <span key={sid} style={{display:"flex",alignItems:"center",gap:5,background:"rgba(255,255,255,0.08)",borderRadius:20,padding:"3px 10px"}}>
                <span style={{width:6,height:6,borderRadius:"50%",background:s.color,display:"inline-block"}}/><span style={{fontSize:10,color:"rgba(255,255,255,0.5)"}}>{s.label}</span><TagBadge type={type}/>
              </span>
            );})}
          </div>
        </div>
        <div style={{padding:"20px 24px",display:"flex",flexDirection:"column",gap:14}}>
          {criterion.detail.map(f=>(
            <div key={f.id}>
              ...
              {f.type==="textarea"?<textarea value={form[f.id]||""} onChange={e=>setForm(p=>({...p,[f.id]:e.target.value}))} placeholder={f.placeholder} style={{width:"100%",minHeight:72,padding:"8px 11px",borderRadius:8,border:"1.5px solid #E2E8F0",fontSize:13,fontFamily:"inherit",resize:"vertical",outline:"none",lineHeight:1.6}} onFocus={e=>e.target.style.borderColor="#1B4FD8"} onBlur={e=>e.target.style.borderColor="#E2E8F0"}/>
              :f.type==="select"?<select value={form[f.id]||""} onChange={e=>setForm(p=>({...p,[f.id]:e.target.value}))} style={{width:"100%",padding:"8px 11px",borderRadius:8,border:"1.5px solid #E2E8F0",fontSize:13,fontFamily:"inherit",outline:"none",appearance:"none",background:"#F8FAFF"}}><option value="">Select...</option>{f.options.map(o=><option key={o} value={o}>{o}</option>)}</select>
              :<input type={f.type} value={form[f.id]||""} onChange={e=>setForm(p=>({...p,[f.id]:e.target.value}))} placeholder={f.placeholder||""} style={{width:"100%",padding:"8px 11px",borderRadius:8,border:"1.5px solid #E2E8F0",fontSize:13,fontFamily:"inherit",outline:"none"}} onFocus={e=>e.target.style.borderColor="#1B4FD8"} onBlur={e=>e.target.style.borderColor="#E2E8F0"}/>}
            </div>
          ))}
        </div>
...
const PH = {
  ...
  deccriteria:{meeting:"What must be true to say yes?",poc:"Criteria shift?",nego:"New criteria?"},
...
// ── MATRIX CELL ───────────────────────────────────────────────────────────────
function MatrixCell({deal,criterion,stageId,onNoteChange}) {
  ...
  return (
    <td style={{verticalAlign:"top",borderRight:"1px solid #EEF0F4",padding:"8px 10px",minWidth:148}}>
      ...
      <textarea value={note} onChange={e=>onNoteChange(criterion.id,stageId,e.target.value)} placeholder={PH[criterion.id]?.[stageId]||"Notes..."}
        style={{width:"100%",minHeight:50,fontSize:11,fontFamily:"inherit",border:"1px solid #E2E8F0",borderRadius:6,padding:"5px 7px",resize:"none",outline:"none",lineHeight:1.5,background:note?"#fff":"#F8FAFF"}}
...
function DealView({deal,onBack,onUpdate,onSaveStatus}) {
  ...
  const [detailCrit,setDetailCrit]=useState(null);
  ...
  function handleNoteChange(cid,sid,val) {
    scheduleUpdate({...deal,criteria:{...deal.criteria,[cid]:{...deal.criteria[cid],stages:{...(deal.criteria[cid]?.stages||{}),[sid]:{note:val}}}}});
  }

  function handleDetailSave(cid,data) {
    scheduleUpdate({...deal,criteria:{...deal.criteria,[cid]:{...deal.criteria[cid],detail:data}}});
  }

  async function saveHeader() {
    const updated={...deal,...header};
    onUpdate(updated);setEditingHeader(false);
    try{await updateDeal(updated);onSaveStatus("saved");}catch{onSaveStatus("error");}
  }

  const {filled,total}=countFilled(deal);
  const pct=total?Math.round((filled/total)*100):0;
  const vis=activeStage?STAGES.filter(s=>s.id===activeStage):STAGES;
    ...
    <div style={{minHeight:"100vh",background:"#F4F3EF",fontFamily:"'DM Sans',sans-serif"}}>
      <div style={{background:"#0F172A",padding:"11px 22px",display:"flex",alignItems:"center",justifyContent:"space-between",position:"sticky",top:0,zIndex:100}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:12}}>
          ...
          <div style={{width:64,height:3,background:"rgba(255,255,255,0.1)",borderRadius:2,overflow:"hidden"}}><div style={{width:`${pct}%`,height:"100%",background:"#1D9E75",borderRadius:2}}/></div>
      ...
      <div style={{background:"#fff",borderBottom:"1px solid #E2E8F0",padding:"0 22px",position:"sticky",top:46,zIndex:90,display:"flex",gap:0,overflowX:"auto"}}>
        ...
        {STAGES.map(s=>(
          <button key={s.id} onClick={()=>setActiveStage(activeStage===s.id?null:s.id)} style={{padding:"10px 14px",border:"none",borderBottom:activeStage===s.id?`2px solid ${s.color}`:"2px solid transparent",background:"transparent",fontSize:12,fontWeight:500,cursor:"pointer",color:activeStage===s.id?s.color:"#94A3B8",whiteSpace:"nowrap",fontFamily:"inherit",display:"flex",alignItems:"center",gap:5}}>
          ...
          <table style={{borderCollapse:"collapse",width:"100%",fontSize:12}}>
              ...
              <tr style={{background:"#F8FAFF",borderBottom:"2px solid #E2E8F0"}}>
                ...
                {vis.map(s=>(
                  <th key={s.id} style={{padding:"10px 12px",textAlign:"center",borderLeft:"1px solid #EEF0F4",borderTop:`3px solid ${s.color}`}}>
                    <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:5}}><span style={{width:6,height:6,borderRadius:"50%",background:s.color,display:"inline-block"}}/><span style={{fontSize:12,fontWeight:600,color:"#0F172A"}}>{s.label}</span></div>
            ...
            <tbody>
              {CRITERIA.map((group,gi)=>(
                <>
                  <tr key={`g${gi}`}>
                    <th colSpan={vis.length+1} style={{padding:"8px 12px 6px",background:"#F8FAFF",borderTop:gi>0?"2px solid #E2E8F0":"none",textAlign:"left",fontSize:9.5,fontWeight:600,textTransform:"uppercase",letterSpacing:"0.08em",color:"#94A3B8"}}>
                    ...
                    return(
                      <tr key={criterion.id} style={{borderBottom:"1px solid #F1F5F9"}}>
                        <td style={{padding:"10px 12px",verticalAlign:"top",position:"sticky",left:0,background:"#fff",zIndex:1,borderRight:"1px solid #E2E8F0",width:168}}>
                          ...
                          <button onClick={()=>setDetailCrit(criterion)} style={{marginTop:7,fontSize:10,padding:"2px 8px",borderRadius:4,border:"1px solid #E2E8F0",background:hasDetail?"#0F172A":"#F8FAFF",cursor:"pointer",color:hasDetail?"#fff":"#64748B",fontFamily:"inherit",fontWeight:500}}>{hasDetail?"✓ Detail":"⊕ Detail"}</button>
                        </td>
                        {vis.map(s=><MatrixCell key={s.id} deal={deal} criterion={criterion} stageId={s.id} onNoteChange={handleNoteChange}/>)}
                      </tr>
                    );
                  })}
                </>
      ...
      {editingHeader&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.4)",zIndex:500,display:"flex",alignItems:"center",justifyContent:"center"}}>
          <div style={{background:"#fff",borderRadius:14,padding:26,width:"100%",maxWidth:420,boxShadow:"0 20px 60px rgba(0,0,0,0.15)"}}>
            ...
            {[["company","Company name"],["contact","Primary contact"],["ae","Account Executive"]].map(([k,l])=>(
              <div key={k} style={{marginBottom:13}}>
                ...
                <input value={header[k]} onChange={e=>setHeader(p=>({...p,[k]:e.target.value}))} style={{width:"100%",padding:"9px 12px",borderRadius:8,border:"1.5px solid #E2E8F0",fontSize:13,fontFamily:"inherit",outline:"none"}}/>
              </div>
            ))}
            <div style={{marginBottom:18}}>
              ...
              <select value={header.stage} onChange={e=>setHeader(p=>({...p,stage:e.target.value}))} style={{width:"100%",padding:"9px 12px",borderRadius:8,border:"1.5px solid #E2E8F0",fontSize:13,fontFamily:"inherit",outline:"none",appearance:"none"}}>
      ...
      {detailCrit&&<CriterionDetail deal={deal} criterion={detailCrit} onClose={()=>setDetailCrit(null)} onSave={handleDetailSave}/>}
    </div>
  );
}
...
function DealList({deals,loading,onSelect,onNew,onDelete}) {
  ...
  const filtered=deals.filter(d=>[d.company,d.contact,d.ae].some(v=>v.toLowerCase().includes(search.toLowerCase())));
  return (
    <div style={{minHeight:"100vh",background:"#F4F3EF",fontFamily:"'DM Sans',sans-serif"}}>
        ...
        <div style={{maxWidth:960,margin:"0 auto"}}>
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:22}}>
            ...
            <span style={{marginLeft:"auto",fontSize:11,color:"rgba(255,255,255,0.3)",display:"flex",alignItems:"center",gap:5}}>
              <span style={{width:6,height:6,borderRadius:"50%",background:"#1D9E75",display:"inline-block"}}/>Synced to Airtable
            </span>
          </div>
          <div style={{display:"flex",alignItems:"flex-end",justifyContent:"space-between",flexWrap:"wrap",gap:16}}>
            <div>
              ...
              <div style={{fontSize:13,color:"rgba(255,255,255,0.35)",marginTop:6}}>{loading?"Loading from Airtable...":`${deals.length} deal${deals.length!==1?"s":""} · shared across team`}</div>
            </div>
            <button onClick={onNew} style={{background:"#fff",color:"#0F172A",border:"none",borderRadius:9,padding:"10px 20px",fontSize:13,fontWeight:600,cursor:"pointer",fontFamily:"inherit"}}>+ New deal</button>
          </div>
        </div>
      </div>
      <div style={{maxWidth:960,margin:"0 auto",padding:"24px 32px"}}>
        ...
        {loading?(
          <div style={{textAlign:"center",padding:"60px 0",color:"#94A3B8"}}><div style={{fontSize:24,marginBottom:10,animation:"spin 1s linear infinite"}}>↻</div><div style={{fontSize:14}}>Loading from Airtable...</div></div>
        ):filtered.length===0?(
          <div style={{textAlign:"center",padding:"60px 0",color:"#94A3B8"}}><div style={{fontSize:36,marginBottom:12}}>📋</div><div style={{fontSize:15,fontWeight:500,marginBottom:6}}>{search?"No deals match":"No deals yet"}</div>{!search&&<div style={{fontSize:13}}>Click "+ New deal" to start</div>}</div>
        ):(
            ...
            {filtered.map(deal=>{
              ...
              return(
                <div key={deal.id} onClick={()=>onSelect(deal.id)}
                  style={{background:"#fff",borderRadius:12,padding:20,border:"1px solid #E2E8F0",cursor:"pointer",boxShadow:"0 1px 4px rgba(0,0,0,0.04)",transition:"box-shadow 0.15s,transform 0.15s"}}
                  ...
                  <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12}}>
                    <span style={{display:"flex",alignItems:"center",gap:6,background:"#F8FAFF",borderRadius:20,padding:"3px 10px",border:"1px solid #E2E8F0"}}>
                      <span style={{width:7,height:7,borderRadius:"50%",background:stage.color,display:"inline-block"}}/><span style={{fontSize:11,fontWeight:500,color:"#475569"}}>{stage.label}</span>
                    </span>
                    <button onClick={e=>{e.stopPropagation();if(confirm(`Delete "${deal.company||"this deal"}"?`))onDelete(deal.id);}} style={{background:"transparent",border:"none",cursor:"pointer",color:"#CBD5E1",fontSize:14,padding:"2px 4px"}}>✕</button>
                  </div>
                  ...
                  <div style={{marginTop:14}}>
                    <div style={{display:"flex",justifyContent:"space-between",fontSize:10,color:"#94A3B8",marginBottom:5}}>
                      <span>Qualification progress</span><span style={{fontWeight:600,color:pct>60?"#1D9E75":pct>30?"#BA7517":"#94A3B8"}}>{pct}%</span>
                    </div>
                    <div style={{height:3,background:"#F1F5F9",borderRadius:2,overflow:"hidden"}}><div style={{height:"100%",width:`${pct}%`,background:pct>60?"#1D9E75":pct>30?"#BA7517":"#CBD5E1",borderRadius:2,transition:"width 0.4s"}}/></div>
                  </div>
...
export default function App() {
  ...
  useEffect(()=>{
    fetchAllDeals().then(data=>{setDeals(data);setLoading(false);})
      .catch(e=>{console.error(e);showToast("Could not load from Airtable","error");setLoading(false);});
  },[]);
  ...
  async function handleNew() {
    const d=newDeal();
    try{const saved=await createDeal(d);setDeals(prev=>[saved,...prev]);setActive(saved.id);}
    catch(e){console.error(e);showToast("Failed to create deal","error");}
  }

  function handleUpdate(updated){setDeals(prev=>prev.map(d=>d.id===updated.id?updated:d));}

  async function handleDelete(id) {
    try{await deleteDeal(id);setDeals(prev=>prev.filter(d=>d.id!==id));if(activeDealId===id)setActive(null);showToast("Deal deleted");}
  ...
  return (
    <>
      {activeDeal
        ?<DealView deal={activeDeal} onBack={()=>setActive(null)} onUpdate={handleUpdate} onSaveStatus={s=>{if(s==="saved")showToast("Saved to Airtable");if(s==="error")showToast("Save failed","error");}}/>
